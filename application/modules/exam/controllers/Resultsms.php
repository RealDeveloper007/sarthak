...
class Resultsms extends MY_Controller {
    ...
    * ********************************************************** */
    private function _send_sms($data) {
        ...
        $students = $this->sms->get_student_list($this->input->post('receiver_id'), $class_id);
        ...
        foreach ($students as $obj) {
           ...
           if($data['role_id'] == GUARDIAN){ 
            ...
            $result = $this->sms->get_single('final_results',array('student_id'=> $obj->student_id, 'class_id'=>$this->input->post('class_id'), 'academic_year_id'=> $this->academic_year_id));
            
            if(!empty($result) && $obj->phone != ' '){
                $grade = $this->sms->get_single('grades',array('id'=> $result->grade_id)); 
                ...
                $message .= 'GPA:'.$result->avg_grade_point.', '; 
