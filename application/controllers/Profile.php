...
class Profile extends My_Controller {
    ...
    public function index() {
        ...
        $admin_id = $this->session->userdata('id');
            ...
            if($role_id == ADMIN)
            {
                $this->data['profile'] = $this->db->get_where('users', array('id'=>$admin_id))->row();
            } else if ($role_id == TEACHER) {
             $this->data['profile'] = $this->db->get_where('users', array('id'=>$profile_id))->row();
            }
            ...
            if ($this->input->post('user_type') == 'teacher') {
                ...
                $data = $_POST;
                 ...
                 if ($_FILES['signature']['name']) {
                    ...
                    unset($data['prev_signature']);
                    unset($data['user_type']);
     
                $updated = $this->profile->update('users', $data, array('id' => $this->input->post('id')));
    ...
    private function _upload_photo() {
        ...
        /**     * *************Function _upload_sign**********************************
     * @type            : Function
     * @function name   : _upload_sign
     * @description     : this private function used to upload user profile sign            
     * @param           : null; 
     * @return          : null 
     * ********************************************************** */
    private function _upload_signature() {
        ...
        if ($signature != "") {
            if ($signature_type == 'image/jpeg' || $signature_type == 'image/pjpeg' ||
                ...
                $destination = 'assets/uploads/' . $this->input->post('user_type') . '-sign/';

                $file_type = explode(".", $signature);
                $extension = strtolower($file_type[count($file_type) - 1]);
                $sign_path = 'sign-' . time() . '-sms.' . $extension;

                move_uploaded_file($_FILES['signature']['tmp_name'], $destination . $sign_path);
                ...
                $return_signature = $sign_path;
            }
        } else {
            $return_photo = $prev_signature;
        }
